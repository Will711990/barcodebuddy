...
SERVER_ADDRESS="${SERVER_ADDRESS:="http://192.168.1.27/api/"}"
...
API_KEY="${API_KEY:="a8tPj5Ivkg3O90pnUxXMWDVE7fJAiB"}"
...
if [ $# -eq 0 ]; then
    nInputEvents=$(ls 2>/dev/null -Ubad1 -- /dev/input/by-id/usb-413d_2107* | wc -l)
        if [ $nInputEvents = 1 ]; then
            deviceToUse=$(ls /dev/input/by-id/usb-413d_2107* | head -n 1)
        else
