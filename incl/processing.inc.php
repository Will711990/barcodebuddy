...
function createLogModeChange(int $state): string {
    ...
    switch ($state) {
        case STATE_CONSUME:
            $text .= "Consume";
            shell_exec ('bash /var/www/html/barcodebuddy/incl/mode_conso.sh');
            break;
        ...
        case STATE_PURCHASE:
            $text .= "Purchase";
            shell_exec ('bash /var/www/html/barcodebuddy/incl/mode_achat.sh');
            break;
        case STATE_OPEN:
            $text .= "Open";
            break;
...
function processUnknownBarcode(string $barcode, bool $websocketEnabled, LockGenerator &$fileLock, ?string $bestBeforeInDays, ?string $price): string {
    ...
    shell_exec ('bash /var/www/html/barcodebuddy/incl/code_barre_inconnu.sh');
...
function processKnownBarcode(GrocyProduct $productInfo, string $barcode, bool $websocketEnabled, LockGenerator &$fileLock, ?string $bestBeforeInDays, ?string $price): string {
    ...
    switch ($state) {
        ...
        case STATE_CONSUME:
            ...
            shell_exec ('bash /var/www/html/barcodebuddy/incl/code_barre_connu.sh');
            ...
            if ($QuantiteAConsommer <> "Barcode") {
                $amountToConsume = $quick_consume_amount;
                $etat = 1;
            } else {
            ...
            if ($productInfo->stockAmount > 0) {
                ...
                $log    = new LogOutput("Etat : " . $etat  . " - Consuming " . $amountToConsume . " " . $productInfo->unit . " of " . $productInfo->name, EVENT_TYPE_CONSUME_PRODUCT, $barcode);
                $output = $log
